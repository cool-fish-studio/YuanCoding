...
module.exports.setSession = function (req, res, next) {
    // var passMd5 = helper.getMd5(req.params.id + config.MD5_SUFFIX);
    // if (!req.cookies[passMd5])
    //     res.cookie(passMd5, 1, {httpOnly: true});
    var userID = helper.getObjectID().toString();
    // if (!req.cookies['yuancoding_session'])
    //     res.cookie('yuancoding_session', userID, {expires: new Date(Date.now() + 86400000*30), httpOnly: true});
    if (!req.cookies['yuancoding_userID'])
